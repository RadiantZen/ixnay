...
{
  options = {
    ixnay.programs.gaming.vkbasalt.enable = lib.mkEnableOption "vkbasalt";
  };

  config = lib.mkIf config.ixnay.programs.gaming.vkbasalt.enable {
    environment = {
      systemPackages = with pkgs; [ vkbasalt ];
    };
  };
}
# TODO Wrap
